...
@Lazy
@Component
public class RepoCacheDBConnectorImplDao {

	static Logger cachedbLogger = LoggerFactory.getLogger(RepoCacheDBConnectorImplDao.class);
...
//	DateUtil oDateUtils;
	...
	public JsonNode GetProcessConfig(@Simple("${body[ProcessConfigRequest][projectCode]}") String projectCode,
		...
		try {
			...
			ObjectNode oProcessObjNode = oProcessConfigResponse.putObject("processConfig");
			ObjectNode oProcessParamsObjNode = oProcessConfigResponse.putObject("processParams");
			ArrayNode processParamArryNode = oProcessParamsObjNode.putArray("processParamConfig");
...
//				System.out.println("RS TEMP"+rs.getString(0));
	...
	public JsonNode GetCache(@Simple("${body[CacheRequest][cacheKey]}") String cacheKey,
			@Simple("${body[CacheRequest][processCode]}") Integer processCode, Exchange exchange)
			throws SQLException {
		...
		try {
			...
			String query = "SELECT CACHE_KEY,PROCESS_CODE,PARAM_CODE1,PARAM_VALUE1,PARAM_CODE2,PARAM_VALUE2,PARAM_CODE3,PARAM_VALUE3,PARAM_CODE4,PARAM_VALUE4,PARAM_CODE5,PARAM_VALUE5,PARAM_CODE6,PARAM_VALUE6,PARAM_CODE7,\r \n"
					+ "PARAM_VALUE7,PARAM_CODE8,PARAM_VALUE8,CREATED_ON,PARTITION_MONTH,STATUS,RESPONSE_OBJECT,ERROR_COUNT,RESPONSE_TYPE FROM DH_REPO_CACHE_PROCESS_DATA where CACHE_KEY= ? AND PROCESS_CODE=? ";
			...
			if (oAppLogger != null) {
				oAppLogger.auditAPIRequest("CacheRequest", "CacheRequest", exchange.getIn().getBody(String.class),
						null, exchange);
			}
			...
			ObjectNode oCacheDataResponseObjNode = oGetCacheResponseObjNode.putObject("CacheResponse");
			...
			while (rs.next()) {
				...
				oCacheDataResponseObjNode.put("CREATED_ON", rs.getString(19));
				oCacheDataResponseObjNode.put("PARTITION_MONTH", rs.getString(20));
				oCacheDataResponseObjNode.put("STATUS", rs.getString(21));
				oCacheDataResponseObjNode.put("RESPONSE_OBJECT", rs.getString(22));
				oCacheDataResponseObjNode.put("ERROR_COUNT", rs.getInt(23));
				oCacheDataResponseObjNode.put("RESPONSE_TYPE", rs.getString(24));

			}

			oGetCacheResponseObjNode.set("CacheResponse", oCacheDataResponseObjNode);

			if (oAppLogger != null) {
				oAppLogger.auditAPIResponse("CacheResponse", "CacheResponse", null,
	 ...
	 * @throws SQLException
	 */
	public JsonNode AddCacheAudit(@Simple("${body[CacheAuditCreationRequest][processCode]}") Integer processCode,
			@Simple("${body[CacheAuditCreationRequest][code]}") String code,
			@Simple("${body[CacheAuditCreationRequest][description]}") String description,
			@Simple("${body[CacheAuditCreationRequest][processDataRef]}") Integer processDataRef,
			@Simple("${body[CacheAuditCreationRequest][paramCode1]}") String paramCode1,
			@Simple("${body[CacheAuditCreationRequest][paramValue1]}") String paramValue1,
			@Simple("${body[CacheAuditCreationRequest][paramCode2]}") String paramCode2,
			@Simple("${body[CacheAuditCreationRequest][paramValue2]}") String paramValue2,
			@Simple("${body[CacheAuditCreationRequest][paramCode3]}") String paramCode3,
			@Simple("${body[CacheAuditCreationRequest][paramValue3]}") String paramValue3,
			@Simple("${body[CacheAuditCreationRequest][paramCode4]}") String paramCode4,
			@Simple("${body[CacheAuditCreationRequest][paramValue4]}") String paramValue4,
			@Simple("${body[CacheAuditCreationRequest][paramCode5]}") String paramCode5,
			@Simple("${body[CacheAuditCreationRequest][paramValue5]}") String paramValue5,
			@Simple("${body[CacheAuditCreationRequest][paramCode6]}") String paramCode6,
			@Simple("${body[CacheAuditCreationRequest][paramValue6]}") String paramValue6,
			@Simple("${body[CacheAuditCreationRequest][paramCode7]}") String paramCode7,
			@Simple("${body[CacheAuditCreationRequest][paramValue7]}") String paramValue7,
			@Simple("${body[CacheAuditCreationRequest][paramCode8]}") String paramCode8,
			@Simple("${body[CacheAuditCreationRequest][paramValue8]}") String paramValue8, Exchange exchange)
			throws SQLException {
...
//			SimpleDateFormat oDateFormat = new SimpleDateFormat("yyyy-MM-dd'T'hh:mm:ss.S");
			...
			oPreparedStatement.setTimestamp(4,new java.sql.Timestamp(new Date().getTime()));
			...
			if (oAppLogger != null) {
				oAppLogger.auditAPIRequest("CacheAuditCreationRequest", "CacheAuditCreationRequest",
						exchange.getIn().getBody(String.class), null, exchange);
			}
			...
			if (rowsAffected > 0) {
				...
				ObjectNode oCacheAuditDataObjNode = oCacheAuditObjNode.putObject("CacheAuditCreationResponse");

				if (rs.next()) {
					Integer generatedId = rs.getInt(1); // retrieved value of the id

					oCacheAuditDataObjNode.put("id", generatedId);
					oCacheAuditDataObjNode.put("processCode", processCode);
					oCacheAuditDataObjNode.put("processDataRef", processDataRef);
				}

				if (oAppLogger != null) {
					oAppLogger.auditAPIResponse("CacheAuditCreationResponse", "CacheAuditCreationResponse", null,
	 ...
	 * @throws SQLException
	 */
	public JsonNode DeleteCache(@Simple("${header.processCode}") Integer processCode,
			@Simple("${header.cacheKey}") String cacheKey, Exchange exchange) throws SQLException {
		...
		try {
			conn = cacheDataSource.getConnection();
			String query = "DELETE FROM DH_REPO_CACHE_PROCESS_DATA WHERE PROCESS_CODE= :PROCESS_CODE AND CACHE_KEY= :CACHE_KEY";
			...
			if (oAppLogger != null) {
				oAppLogger.auditAPIRequest("CacheDeletionRequest", "CacheDeletionRequest",
						exchange.getIn().getHeader("processCode", Integer.class)
								+ exchange.getIn().getHeader("cacheKey", String.class),
						null, exchange);
			}
			...
			if (rowDeleted > 0) {
				oDeleteCacheResponseObjNode.put("cacheKey", cacheKey);
			}
	...
	public JsonNode UpdateCache(@Simple("${body[CacheUpdationRequest][status]}") String status,
			@Simple("${body[CacheUpdationRequest][processCode]}") Integer processCode,
			@Simple("${body[CacheUpdationRequest][cacheKey]}") String cacheKey,
			@Simple("${body[CacheUpdationRequest][errorCount]}") Integer errorCount, Exchange exchange)
			throws SQLException {
		...
		try {
			...
			if(status != null && errorCount != null ) {
				query.append("UPDATE DH_REPO_CACHE_PROCESS_DATA SET STATUS= :STATUS,ERROR_COUNT= :ERROR_COUNT WHERE PROCESS_CODE= :PROCESS_CODE AND CACHE_KEY= :CACHE_KEY");
			}else if(status == null && errorCount != null ) {
				query.append("UPDATE DH_REPO_CACHE_PROCESS_DATA SET ERROR_COUNT= :ERROR_COUNT WHERE PROCESS_CODE= :PROCESS_CODE AND CACHE_KEY= :CACHE_KEY");
			}
			oPreparedStatement = conn.prepareStatement(query.toString());
			if(status != null && errorCount != null ) {
				oPreparedStatement.setString(1, status);
				oPreparedStatement.setInt(2, errorCount);
				oPreparedStatement.setInt(3, processCode);
				oPreparedStatement.setString(4, cacheKey);
			}else if(status == null && errorCount != null){
				oPreparedStatement.setInt(1, errorCount);
				oPreparedStatement.setInt(2, processCode);
			...
			rowUpdated = oPreparedStatement.executeUpdate();
			...
			if (rowUpdated > 0) {
				...
				oUpdateCacheResponseObjNode.put("cacheKey", cacheKey);
			}
	 ...
	 * @throws SQLException
	 */
	public JsonNode AddCache(@Simple("${body[CacheAdditionRequest][cacheKey]}") String cacheKey,
			@Simple("${body[CacheAdditionRequest][processCode]}") Integer processCode,
			@Simple("${body[CacheAdditionRequest][paramCode1]}") String paramCode1,
			@Simple("${body[CacheAdditionRequest][paramValue1]}") String paramValue1,
			@Simple("${body[CacheAdditionRequest][paramCode2]}") String paramCode2,
			@Simple("${body[CacheAdditionRequest][paramValue2]}") String paramValue2,
			@Simple("${body[CacheAdditionRequest][paramCode3]}") String paramCode3,
			@Simple("${body[CacheAdditionRequest][paramValue3]}") String paramValue3,
			@Simple("${body[CacheAdditionRequest][paramCode4]}") String paramCode4,
			@Simple("${body[CacheAdditionRequest][paramValue4]}") String paramValue4,
			@Simple("${body[CacheAdditionRequest][paramCode5]}") String paramCode5,
			@Simple("${body[CacheAdditionRequest][paramValue5]}") String paramValue5,
			@Simple("${body[CacheAdditionRequest][paramCode6]}") String paramCode6,
			@Simple("${body[CacheAdditionRequest][paramValue6]}") String paramValue6,
			@Simple("${body[CacheAdditionRequest][paramCode7]}") String paramCode7,
			@Simple("${body[CacheAdditionRequest][paramValue7]}") String paramValue7,
			@Simple("${body[CacheAdditionRequest][paramCode8]}") String paramCode8,
			@Simple("${body[CacheAdditionRequest][paramValue8]}") String paramValue8,
			@Simple("${body[CacheAdditionRequest][createdBy]}") String createdBy,
			@Simple("${body[CacheAdditionRequest][status]}") String status,
			@Simple("${body[CacheAdditionRequest][responseObject]}") String responseObject,
			@Simple("${body[CacheAdditionRequest][errorCount]}") Integer errorCount,
			@Simple("${body[CacheAdditionRequest][responseType]}") String responseType, Exchange exchange)
			throws SQLException {
...
//			SimpleDateFormat oDateFormat = new SimpleDateFormat("yyyy-MM-dd'T'hh:mm:ss.S");
			...
			String query = "INSERT INTO DH_REPO_CACHE_PROCESS_DATA(CACHE_KEY,PROCESS_CODE,PARAM_CODE1,PARAM_VALUE1,PARAM_CODE2,PARAM_VALUE2,PARAM_CODE3,PARAM_VALUE3,PARAM_CODE4,PARAM_VALUE4,PARAM_CODE5,PARAM_VALUE5,PARAM_CODE6,PARAM_VALUE6,PARAM_CODE7,PARAM_VALUE7,"
					...
					+ "VALUES(:CACHE_KEY,:PROCESS_CODE,:PARAM_CODE1,:PARAM_VALUE1,:PARAM_CODE2,:PARAM_VALUE2,:PARAM_CODE3,:PARAM_VALUE3,:PARAM_CODE4,:PARAM_VALUE4,:PARAM_CODE5,:PARAM_VALUE5,:PARAM_CODE6,:PARAM_VALUE6,:PARAM_CODE7,:PARAM_VALUE7,"
					+ ":PARAM_CODE8,:PARAM_VALUE8,:CREATED_ON,:CREATED_BY,:STATUS,:RESPONSE_OBJECT,:ERROR_COUNT,:RESPONSE_TYPE)";
			...
			oPreparedStatement.setTimestamp(19,new java.sql.Timestamp(new Date().getTime()));
			...
			if (oAppLogger != null) {
				oAppLogger.auditAPIRequest("CacheAdditionRequest", "CacheAdditionRequest", exchange.getIn().getBody(String.class),
						null, exchange);
			}
			...
			ObjectNode oAddCacheResponseObjNode = oAddCacheObjNode.putObject("CacheAdditionResponse");
			if (rowInserted > 0) {
				rs = oPreparedStatement.getGeneratedKeys();
				if (rs.next()) {
					Integer generatedId = rs.getInt(1);
					oAddCacheResponseObjNode.put("id", generatedId);
					oAddCacheResponseObjNode.put("cacheKey", cacheKey);
				}
			}

			if (oAppLogger != null) {
				oAppLogger.auditAPIResponse("CacheAdditionResponse", "CacheAdditionResponse", null, oAddCacheObjNode.toString(),
						exchange);
			}
	...
	public JsonNode AddProject(@Simple("${body[ProjectCreationRequest][projectCode]}") String projectCode,
		...
		try {
			...
			oPreparedStatement.setTimestamp(3,new java.sql.Timestamp(new Date().getTime()));
	...
	public JsonNode UpdateProject(@Simple("${body[ProjectUpdationRequest][projectCode]}") String projectCode,
		...
		try {
			...
			oPreparedStatement.setTimestamp(2,new java.sql.Timestamp(new Date().getTime()));
	...
	public JsonNode DeleteProject(@Simple("${header.projectCode}") String projectCode, Exchange exchange)
		...
		try {
			...
			cachedbLogger.error(oException.getMessage());
			//throw new Exception(String.valueOf(oException.getErrorCode()));
			throw new Exception(oException.getSQLState());
		} finally {
			if (null != oPreparedStatement)
				oPreparedStatement.close();
			if (conn != null)
	...
	public JsonNode AddProcess(@Simple("${body[ProcessCreationRequest][projectCode]}") String projectCode,
		...
		try {
			...
			oPreparedStatement.setTimestamp(14,new java.sql.Timestamp(new Date().getTime()));
	...
	public JsonNode GetProcessParams(@Simple("${header.processId}") Integer processId,
		...
		try {
			...
			query.append("SELECT PROCESS_CODE AS PROCESS_ID,PARAM_CODE,PARAM_XPATH,PARAM_TYPE,PARAM_FORMAT,PARAM_ORDER,"
					...
					+ "IS_ERROR_VARIABLE,CREATED_ON,CREATED_BY,MODIFIED_ON,MODIFIED_BY FROM DH_REPO_CACHE_PROCESS_PARAMS");
	...
	public JsonNode AddProcessParams(@Simple("${body[ProcessParamsCreationRequest][processId]}") Integer processId,
		...
		try {
			...
			oPreparedStatement.setString(15, createdBy);
			oPreparedStatement.setTimestamp(16,new java.sql.Timestamp(new Date().getTime()));
